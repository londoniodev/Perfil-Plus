...
describe('PaymentsController', () => {
    ...
    describe('createSubscriptionCheckout', () => {
        it('debería crear checkout de suscripción con MercadoPago', async () => {
            mockPaymentsService.createSubscriptionCheckout.mockResolvedValue(mockCheckoutResult);

            const result = await controller.createSubscriptionCheckout(
                'user-1',
                { email: 'user@test.com', frontUrl: 'http://localhost:3000' },
            );
    ...
    describe('getSubscriptionStatus', () => {
        it('debería retornar estado de suscripción del usuario', async () => {
            mockPaymentsService.getSubscriptionStatus.mockResolvedValue(mockSubscriptionStatus);

            const result = await controller.getSubscriptionStatus('user-1');

            expect(mockPaymentsService.getSubscriptionStatus).toHaveBeenCalledWith('user-1');
    ...
    describe('cancelSubscription', () => {
        it('debería cancelar suscripción del usuario', async () => {
            mockPaymentsService.cancelSubscription.mockResolvedValue({ status: 'CANCELLED' });

            const result = await controller.cancelSubscription('user-1');

            expect(mockPaymentsService.cancelSubscription).toHaveBeenCalledWith('user-1');
        });
    });
    ...
    describe('handleWebhook', () => {
        it('debería verificar firma y procesar webhook de pago aprobado', async () => {
            ...
            mockPaymentsService.handleWebhook.mockResolvedValue({ status: 'processed' });
            ...
            const result = await controller.handleWebhook(body, 'ts=1;v1=abc', 'req-1');
            ...
            expect(mockPaymentsService.handleWebhook).toHaveBeenCalledWith('payment', '12345');
        ...
        it('debería rechazar webhook con firma inválida', async () => {
            ...
            const result = await controller.handleWebhook(body, 'ts=1;v1=FALSA', 'req-1');
        ...
        it('debería ignorar webhook sin type o data.id', async () => {
            ...
            expect(result).toEqual({ status: 'ignored', reason: 'missing type or data.id' });
        });
        ...
        it('debería manejar formato alternativo de webhook (topic + id)', async () => {
            ...
            expect(mockPaymentsService.handleWebhook).toHaveBeenCalledWith('payment', '67890');
        });
    });
