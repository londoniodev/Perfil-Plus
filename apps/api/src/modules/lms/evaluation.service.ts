...
import { CreateEvaluationDto, UpdateEvaluationDto, SubmitEvaluationDto, CreateQuestionDto } from './dto';
...
export class EvaluationService {
    ...
    async createEvaluation(dto: CreateEvaluationDto) {
        ...
        if (theme.evaluation) throw new BadRequestException('Este tema ya tiene una evaluación');
    ...
    async updateEvaluation(id: string, dto: UpdateEvaluationDto) {
        const evaluation = await this.prisma.client.evaluation.findUnique({ where: { id } });
    ...
    async deleteEvaluation(id: string) {
        ...
        await this.prisma.client.evaluation.delete({ where: { id } });
    ...
    async addQuestion(evaluationId: string, dto: CreateQuestionDto) {
        const evaluation = await this.prisma.client.evaluation.findUnique({ where: { id: evaluationId } });
        if (!evaluation) throw new NotFoundException('Evaluación no encontrada');

        // Obtener el orden máximo actual
        const maxOrder = await this.prisma.client.question.aggregate({
            where: { evaluationId },
            _max: { order: true },
        });
    ...
    async updateQuestion(id: string, dto: Partial<CreateQuestionDto>) {
        ...
        return this.prisma.client.question.update({
            where: { id },
            data: dto.options ? { ...dto, options: dto.options as any } : dto as any,
        });
    }
    ...
    async getEvaluationForUser(evaluationId: string, userId: string) {
        ...
        const existingResult = await this.prisma.client.evaluationResult.findUnique({
    ...
    async submitEvaluation(evaluationId: string, userId: string, dto: SubmitEvaluationDto) {
        const evaluation = await this.prisma.client.evaluation.findUnique({
            where: { id: evaluationId },
        ...
        // Guardar resultado
        const result = await this.prisma.client.evaluationResult.create({
            data: {
                userId,
                evaluationId,
                score,
    ...
    async getUserResults(userId: string) {
        return this.prisma.client.evaluationResult.findMany({
            where: { userId },
            include: {
                evaluation: {
                    select: {
