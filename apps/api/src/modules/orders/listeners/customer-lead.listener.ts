  ...
  async handleCustomerLead(event: OrderCreatedEvent) {
    try {
      ...
      if (orderType === 'DELIVERY' || orderType === 'TAKE_AWAY') {
        const shipping = shippingData as any;
        ...
        this.logger.log(`Customer Data upsert para WhatsApp (Phone: ${customerPhone})`);
      }

    } catch (error) {
      this.logger.error(`Error procesando Customer Leads para orden ${event.order?.id}: ${error.message}`, error.stack);
    }
  }
}
