...
export class BlogController {
  ...
  @Get('posts/:slug')
  @Public()
  async getPostBySlug(@Param('slug') slug: string, @CurrentUser() user?: any) {
    const hasSubscription =
      user?.hasActiveSubscription || user?.role === 'ADMIN' || false;
...
export class AdminBlogController {
  ...
  @Delete('posts/:id')
  async deletePost(@Param('id') id: string, @CurrentTenant() tenantId: string) {
  ...
  @Delete('tags/:id')
  async deleteTag(@Param('id') id: string, @CurrentTenant() tenantId: string) {
