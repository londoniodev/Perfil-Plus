    ...
    return (
        <section className="mb-8">
            <div className="flex items-center justify-between px-6 mb-4">
                <h3 className="font-display font-bold text-lg text-gray-900 dark:text-gray-100">Categorías</h3>
            ...
            <div className="flex overflow-x-auto gap-3 px-6 pb-4 no-scrollbar">
                {categories.map((cat) => (
                    <div key={cat.id} className="flex-shrink-0 flex flex-col items-center gap-2 group cursor-pointer transition-transform hover:scale-105">
                        <div className={`w-16 h-16 rounded-2xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center ${cat.colorClass} ${cat.bgHoverClass} group-hover:text-white transition-colors duration-300`}>
                            <span className="material-icons-outlined text-2xl leading-none">
                                {cat.icon}
                            </span>
                        </div>
                        <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-gray-100 transition-colors">
                            {cat.name}
                        </span>
                    </div>
                ))}
