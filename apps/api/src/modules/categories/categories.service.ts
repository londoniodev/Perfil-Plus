...
export class CategoriesService {
    ...
    async update(tenantId: string, id: string, updateDto: UpdateCategoryDto) {
        ...
        if (!existing) {
            throw new NotFoundException(`Categoria con ID ${id} no encontrada en este Tenant`);
        }
        ...
        if (updateDto.name && updateDto.name !== existing.name) {
            ...
            if (slugConflict) throw new BadRequestException('El nombre genera un slug ya en uso.');
        }
    ...
    async remove(tenantId: string, id: string) {
        ...
        if (productCount > 0) {
            throw new BadRequestException(`No se puede eliminar la categoría porque tiene ${productCount} productos asociados.`);
        }
