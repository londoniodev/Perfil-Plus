...
export default function EditarCursoPage({ params }: EditarCursoPageProps) {
    ...
    return (
        <div className="p-6 md:p-8 max-w-4xl mx-auto space-y-8">
            <Link href={`/academia/cursos/temas/${ids!.id}`} className="inline-flex items-center gap-2 text-muted-foreground hover:text-primary transition-colors">
                <IconBack size={16} />
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="flex justify-end gap-3 pt-6 border-t">
                            <Button variant="outline" asChild>
                                <Link href={`/academia/cursos/temas/${ids!.id}`}>Cancelar</Link>
                            </Button>
            ...
            <Card className="border-border">
                <CardHeader className="flex flex-row items-center justify-between">
                    ...
                    <Button asChild size="sm" className="gap-2">
                        <Link href={`/academia/cursos/temas/${ids!.id}/cursos/${ids!.courseId}/lecciones/nuevo`}>
                            <IconPlus size={16} />
                            Agregar Lección
                        </Link>
                    </Button>
                        ...
                        <div className="flex flex-col gap-2">
                            {course.lessons.map((lesson) => (
                                <LessonItem
                                    key={lesson.id}
                                    lesson={lesson}
                                    onEdit={(id) => router.push(`/academia/cursos/temas/${ids!.id}/cursos/${ids!.courseId}/lecciones/${id}`)}
