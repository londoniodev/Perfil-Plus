...
const MOCK_PRODUCTS = [
        ...
        modifierGroups: [
            {
                ...
                maxSelect: 3,
                modifiers: [
                    { id: 'mod-1', name: 'Queso Extra', priceAdjustment: new Decimal(2), stock: 20 },
                    { id: 'mod-2', name: 'Tocino', priceAdjustment: new Decimal(3.5), stock: null },
                ],
            },
        ],
        categories: [
    ...
    },
    {
        ...
        isAvailable: true,
        variants: [
            { id: 'var-3', name: 'Default', price: new Decimal(2.5) },
        ],
        modifierGroups: [],
        categories: [
...
describe('RestaurantService', () => {
    ...
    describe('getPublicMenu', () => {
        ...
        it('debería lanzar NotFoundException si el tenant no existe', async () => {
            ...
            await expect(service.getPublicMenu('no-existe')).rejects.toThrow(NotFoundException);
        });
        ...
        it('debería retornar valores default cuando design no tiene campos', async () => {
            ...
            expect(result.restaurant.slogan).toBe('Bienvenido a nuestro menú digital');
        ...
        it('debería buscar SystemSetting con key TENANT_CONFIG', async () => {
            ...
            expect(mockPrisma._tenantClient.systemSetting.findUnique).toHaveBeenCalledWith({
        ...
        it('debería manejar producto sin categorías (categories = [])', async () => {
            ...
            mockPrisma._tenantClient.product.findMany.mockResolvedValue(productosSinCat);
            mockPrisma.getTenantBySlug.mockResolvedValue(MOCK_TENANT);
        ...
        it('debería manejar producto sin variantes', async () => {
            ...
            mockPrisma._tenantClient.product.findMany.mockResolvedValue(productoSinVariantes);
            mockPrisma.getTenantBySlug.mockResolvedValue(MOCK_TENANT);
