...
export async function CursosNuevos() {
    ...
    return (
        <section className="mb-8">
            <h3 className="px-6 font-display font-bold text-xl text-gray-900  mb-4">
                Nuevos lanzamientos
            </h3>
            {/* Scrollable container with hidden scrollbar */}
            <div className="flex overflow-x-auto gap-4 px-6 pb-6 no-scrollbar">
                {newCourses.map((course) => (
                    <div key={course.id} className="flex-shrink-0 w-64 bg-white  rounded-xl p-3 shadow-sm border border-gray-100  flex gap-3 items-center group cursor-pointer hover:shadow-md transition-shadow">
                        ...
                        <div className="flex flex-col justify-center">
                            ...
                            <h5 className="font-display font-bold text-sm text-gray-900  leading-tight mb-1 group-hover:text-cs-primary transition-colors">
                                {course.title}
                            </h5>
