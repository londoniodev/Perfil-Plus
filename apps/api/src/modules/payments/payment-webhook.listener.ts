...
export class PaymentWebhookListener {
  ...
  async handlePaymentEvent(payload: { tenantId: string; dataId: string; body: any }) {
    this.logger.log(`Processing payment event for tenant: ${payload.tenantId}, dataId: ${payload.dataId}`);
    ...
    await this.cls.runWith({ tenantId: payload.tenantId } as any, async () => {
      try {
        ...
        this.logger.log(`Payment event processed successfully for dataId: ${payload.dataId}`);
      } catch (error) {
        this.logger.error(`Error processing payment event for dataId: ${payload.dataId}`, error.stack);
      }
    });
  }
}
