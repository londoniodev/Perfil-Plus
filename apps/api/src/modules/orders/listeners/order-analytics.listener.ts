...
export class OrderAnalyticsListener {
  ...
  async handleOrderCreatedEvent(event: OrderCreatedEvent) {
    try {
      ...
      const analyticsData: any = { orderId: order.id, pendingAt: order.createdAt };
      ...
      // Important: Using the root PrismaService client without Tenant filtering
      // since OrderDeliveryAnalytics isn't tenant-bound yet, or if it is,
      // it should be automatically handled if tenant isolation works globally.
      // But Since this is background, CLS might not have context. We must use simple queries.
      ...
      this.logger.error(`Error procesando analíticas para orden ${event.order?.id}: ${error.message}`, error.stack);
    }
  }
  ...
  async handleOrderStatusChanged(event: OrderStatusChangedEvent) {
    try {
        ...
        case 'PREPARING':
          if (!record.preparingAt) {
            ...
            updates.timeToPrepare = record.pendingAt
              ? Math.max(0, Math.floor((now.getTime() - record.pendingAt.getTime()) / 60000))
              : 0;
          }
          break;
        case 'READY':
          if (!record.shippedAt) {
            ...
            updates.timeToShip = record.preparingAt
              ? Math.max(0, Math.floor((now.getTime() - record.preparingAt.getTime()) / 60000))
              : 0;
          }
          break;
        case 'SERVED':
      ...
      if (Object.keys(updates).length > 0) {
        ...
        this.logger.log(`Analíticas actualizadas para orden (ID: ${event.orderId})`);
      }
    } catch (error) {
      this.logger.error(`Error actualizando analíticas para orden ${event.orderId}: ${error.message}`, error.stack);
    }
  }
}
