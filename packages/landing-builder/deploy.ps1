...
if (Test-Path $configPath) {
  ...
  foreach ($f in $folders) {
    ...
    $label = (Culture).TextInfo.ToTitleCase($f.Name.Replace('-', ' '))
    $pages += @{ folder = $f.Name; label = $label }
  }
}

if ($pages.Count -eq 0) {
  Write-Host "⚠️ No pages found to deploy." -ForegroundColor Yellow
  exit 0
}
...
foreach ($page in $pages) {
  ...
  if (-not (Test-Path $inputPath)) {
    Write-Host "⚠️ Skipping $f: raw.html not found." -ForegroundColor Yellow
    continue
  }
